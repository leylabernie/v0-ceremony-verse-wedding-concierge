...
export function EditorialGuide({ data }: { data: EditorialGuideData }) {
  return <main className="bg-[#faf8f5] px-6 py-16 text-[#1f1f1f]">
    ...
    <article className="mx-auto max-w-4xl">
      ...
      <p className="mt-5 text-sm text-[#5e4a40]">By <Link href="/about/mini/" className="text-[#7a6841] underline underline-offset-4">Mini</Link> · Updated <time dateTime={data.modified}>{new Date(data.modified).toLocaleDateString("en-US", {month:"long",day:"numeric",year:"numeric",timeZone:"UTC"})}</time></p>
